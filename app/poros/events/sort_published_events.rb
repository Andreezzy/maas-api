module Events
  class SortPublishedEvents
    ...
    def clear_published_events
      Schedule.first.events.where(kind: 1).destroy_all
    end
    ...
    def assign_events_with_no_collisions
      days_of_week.each do |day|
        ...
        if draft_events_per_day.has_key?(day_of_week)
          ...
          Event.create!(schedule_id: Schedule.first.id, user_id: nil, kind: 1, start_time: current_date.change({hour: hours[0].to_i}), end_time: current_date.change({hour: hours[-1].to_i}))
          next
        end
      end
    end
        ...
        if draft_events_per_day.has_key?(day_of_week)
          ...
          while !collisions.empty? do
            ...
            user_hours[user_id].each do |hours|
              start_hour = nil
              puts "start_hour: #{start_hour}"
              hours.each do |hour|
                puts "hour: #{hour}"
                if collisions.exclude?(hour)
                  puts "collisions: #{hour}"
                  if start_hour
                    create_user_event(user_id, current_date.change({hour: start_hour.to_i}), current_date.change({hour: hour.to_i}))
                    puts "CREATING1: #{current_date.change({hour: start_hour.to_i})} | END: #{current_date.change({hour: hour.to_i})}"
                    total_user_hours[user_id] += hour.to_i - start_hour.to_i
                    puts "total_user_hours: #{total_user_hours[user_id]}"
                    start_hour = nil
                  end
                else
                  puts "else: #{hour}"
              ...
              if start_hour
                create_user_event(user_id, current_date.change({hour: start_hour.to_i}), current_date.change({hour: hours[-1].to_i + 1}))
                puts "CREATING2: #{current_date.change({hour: start_hour.to_i})} | END: #{current_date.change({hour: hours[-1].to_i + 1})}"
                total_user_hours[user_id] += (hours[-1].to_i + 1) - start_hour.to_i
                puts "total_user_hours: #{total_user_hours[user_id]}"
              end
            end
    ...
    def assign_events_without_turns
      refresh = Event.joins(:schedule).where('events.kind = 0').where('schedules.id = 5').order(:start_time).group_by {|x| x[:start_time].strftime('%w')}
      days_of_week.each do |day|
        day_of_week = day.strftime('%w')
        current_date = day.to_datetime
        day_schedule = business_hours[day_of_week]
        ...
        if refresh.has_key?(day_of_week)
          user_hours = refresh[day_of_week].each_with_object({}) {|k, h| (h[k.user_id] ||= []).push((k.start_time.strftime('%H')...k.end_time.strftime('%H')).to_a)}
    ...
    def total_user_hours
      @total_user_hours ||= User.joins(:events).where('events.kind = 0 and events.schedule_id = 5').distinct.pluck(:id).to_h {|id| [id, 0]}
    end

    def create_warning_event(start_time, end_time)
      Event.create!(schedule_id: Schedule.first.id, user_id: nil, kind: 1, start_time: start_time, end_time: end_time)
    end

    def create_user_event(user_id, start_time, end_time)
      Event.create!(schedule_id: Schedule.first.id, user_id: user_id, kind: 1, start_time: start_time, end_time: end_time)
    end

    def draft_events_per_day
      @draft_events_per_day ||= Event.joins(:schedule).where('events.kind = 0').where('schedules.id = 5').order(:start_time).group_by {|x| x[:start_time].strftime('%w')}
                                     # .group_by {|x| x[:start_time].strftime('%d')}
    end

    def days_of_week
      @days_of_week ||= (Schedule.first.start_date...Schedule.first.end_date).to_a
    end

    def business_hours
      @business_hours ||= Schedule.first.business_hours.order(:day_of_week)
